...
# Define generator
n_input = 60
n_features = len(col_list) - 1
# batch_size = int(len(data)/5)
batch_size = 64
generator = TimeseriesGenerator(data, data, length=n_input, batch_size=batch_size)
...
# Fit model
history = model.fit(generator, steps_per_epoch=len(generator), epochs=5, verbose=1)
...
fig.write_image("multi_tsg_close_training_loss.png")
...
fig = go.Figure(data=plot_data, layout=plot_layout)
fig.write_image("multi_tsg_close_value.png")